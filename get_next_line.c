...
/*   Updated: 2022/07/20 14:58:17 by liperman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line.h"

char	*ft_free(char *static_var, char *buffer)
{
	char	*temp;
...
char	*ft_next(char *static_var)
{
	...
	result = ft_calloc((ft_strlen(static_var) - i + 1), sizeof(char));
	i++;
	j = 0;
	while (static_var[i])
...
char	*read_file(int fd, char *static_var)
{
	...
	while (byte_read > 0 && !(ft_strchr(buffer, '\n')))
	{
		...
		static_var = ft_free(static_var, buffer);
	}
	free (buffer);
	return (static_var);
}
